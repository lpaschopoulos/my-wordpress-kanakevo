...
class WPLE_Admin
{
          ...
          <li>5. ' . sprintf( __( 'Please wait few minutes and click on %sEnable HTTPS Now%s button', 'wp-letsencrypt-ssl' ), '<strong>', '</strong>' ) . '</li>
            ...
            <div class="wple-success-cols">
              <div>
                ...
                <p>' . sprintf( __( 'Download the certs and send it to your hosting support to install it for you. If you have root SSH access, you can refer our %sDOCS%s for server specific instructions.', 'wp-letsencrypt-ssl' ), '<a href="https://wpencryption.com/docs/" target="_blank">', '</a>' ) . '</p>
              </div>
              <div>
              ...
              <div>
                <h3>' . esc_html__( "Looking for SSL + Security + Performance?", 'wp-letsencrypt-ssl' ) . '</h3>
                <p>' . sprintf(
                __( 'Why pay for an SSL certificate alone when you can get %sSSL%s + %sSecurity Firewall%s + %sCDN Performance%s with our %sSecure Firewall%s Service for even lower cost', 'wp-letsencrypt-ssl' ),
                '<strong>',
                '</strong>',
                '<strong>',
                '</strong>',
  ...
  <?php 
    ...
    public function wple_completed_block( &$html )
    {
        ...
        $headline = esc_html__( 'Woohoo! WP Encryption just saved you several $$$ in SSL Certificate Fee.', 'wp-letsencrypt-ssl' );
        ...
        <div class="wple-completed-review">
          ...
          ' . $renewlink . '
          <small>' . esc_html__( 'Just takes few seconds', 'wp-letsencrypt-ssl' ) . '</small>
        </div>';
